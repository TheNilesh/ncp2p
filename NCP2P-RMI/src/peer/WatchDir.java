...
public class WatchDir implements Runnable{
     ...
     */
    WatchDir(Path dir, boolean recursive,PeerImpl peer) throws IOException {
    	this.peer=peer;
	...
	void readCurrentFiles(File sdir){
		File[] f=sdir.listFiles();
		for(int i=0;i<f.length;i++){
			if(!f[i].isDirectory()){
				peer.fileChanged(f[i],FileInfo.CREATE);
			}
		}
	}
    /**
...
if(kind==ENTRY_CREATE){
	//System.out.format("%s: %s\n", event.kind().name(), child);
	peer.fileChanged(child.toFile(), FileInfo.CREATE);
